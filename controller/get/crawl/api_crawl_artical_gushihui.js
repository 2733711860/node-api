...
const greetingURL = 'https://liunianbanxia.com/2019-story-be-read-online.html';

module.exports = (ctx) => {
  return new Promise((resolve, reject) => {
    getGuShiHuiArtical().then(list => {
      getGuShiHuiContent(list).then(result => {
...
// 获取故事会文章标题
async function getGuShiHuiArtical() {
  return new Promise((resolve, reject) => {
...
// 获取故事会文章正文
async function getGuShiHuiContent(list) {
  return new Promise((resolve, reject) => {
...
async function saveArtical(results) {
  return new Promise(async(resolve, reject) => {
    // 创建表
    let createSql = `create table if not exists artical_gushihui(
      id varchar(500),
      title varchar(500),
      image varchar(500),
      content longtext,
    ...
    let editSql = `insert into artical_gushihui (${str1}) values ? on duplicate key update ${updates}`;
