...
const greetingURL = 'https://liunianbanxia.com/tag/short-story/';

module.exports = (ctx) => {
  return new Promise((resolve, reject) => {
    getFeiMoHuanArtical().then(list => {
      console.log(list)
      getFeiMoHuanContent(list).then(result => {
...
// 获取飞魔幻文章标题
async function getFeiMoHuanArtical() {
  return new Promise((resolve, reject) => {
...
// 获取飞魔幻文章正文
async function getFeiMoHuanContent(list) {
  return new Promise((resolve, reject) => {
...
async function saveArtical(results) {
  return new Promise(async(resolve, reject) => {
    // 创建表
    let createSql = `create table if not exists artical_feimohuan(
      id varchar(500),
      title varchar(500),
      image varchar(500),
      content longtext,
    ...
    let editSql = `insert into artical_feimohuan (${str1}) values ? on duplicate key update ${updates}`;
