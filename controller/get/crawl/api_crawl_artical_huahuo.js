...
const greetingURL = 'https://liunianbanxia.com/2019-fireworks-magazine-read.html';

module.exports = (ctx) => {
  return new Promise((resolve, reject) => {
    getHuaHuoArtical().then(list => {
      getHuaHuoContent(list).then(result => {
...
// 获取花火文章标题
async function getHuaHuoArtical() {
  return new Promise((resolve, reject) => {
...
// 获取花火文章正文
async function getHuaHuoContent(list) {
  return new Promise((resolve, reject) => {
...
async function saveArtical(results) {
  return new Promise(async(resolve, reject) => {
    // 创建每日一语表
    let createSql = `create table if not exists artical_huahuo(
      id varchar(500),
      title varchar(500),
      image varchar(500),
      content longtext,
    ...
    let editSql = `insert into artical_huahuo (${str1}) values ? on duplicate key update ${updates}`;
