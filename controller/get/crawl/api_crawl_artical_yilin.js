...
const greetingURL = 'https://liunianbanxia.com/yilin-magazine-read.html';

module.exports = (ctx) => {
  return new Promise((resolve, reject) => {
    getYiLinArtical().then(list => {
      getYiLinContent(list).then(result => {
...
// 获取意林文章标题
async function getYiLinArtical() {
  return new Promise((resolve, reject) => {
...
// 获取意林文章正文
async function getYiLinContent(list) {
  return new Promise((resolve, reject) => {
...
async function saveArtical(results) {
  return new Promise(async(resolve, reject) => {
    // 创建表
    let createSql = `create table if not exists artical_yilin(
      id varchar(500),
      title varchar(500),
      image varchar(500),
      content longtext,
    ...
    let editSql = `insert into artical_yilin (${str1}) values ? on duplicate key update ${updates}`;
