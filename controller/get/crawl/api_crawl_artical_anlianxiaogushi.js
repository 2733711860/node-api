...
const greetingURL = 'https://liunianbanxia.com/tag/crush/';

module.exports = (ctx) => {
  return new Promise((resolve, reject) => {
    getAnlianArtical().then(list => {
      getAnlianContent(list).then(result => {
...
// 获取暗恋小故事文章标题
async function getAnlianArtical() {
  return new Promise((resolve, reject) => {
...
// 获取暗恋小故事文章正文
async function getAnlianContent(list) {
  return new Promise((resolve, reject) => {
...
async function saveArtical(results) {
  return new Promise(async(resolve, reject) => {
    // 创建表
    let createSql = `create table if not exists artical_anlianxiaogushi(
      id varchar(500),
      title varchar(500),
      image varchar(500),
      content longtext,
    ...
    let editSql = `insert into artical_anlianxiaogushi (${str1}) values ? on duplicate key update ${updates}`;
